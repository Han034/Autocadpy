...
class Application(tk.Tk):
    def __init__(self):
        ...
        self.title("Form and Image Viewer")
    ...
    def create_widgets(self):
        ...
        try:
            image1 = Image.open("pictures/HIS.jpg")
            image1 = image1.resize((500, 300), Image.LANCZOS)
        ...
        try:
            image2 = Image.open("pictures/HIS2.jpg")  # İkinci resim yolu
            image2 = image2.resize((500, 300), Image.LANCZOS)
    ...
    def save_entries_form2(self):
        ...
        try:
            # Determine the path to the script
            script_dir = os.path.dirname(os.path.abspath(__file__))
            script_path = os.path.join(script_dir, "HIS.py")

            result = subprocess.run(["python", "-Xfrozen_modules=off", script_path], check=True, capture_output=True, text=True)
            self.output_text.insert(tk.END, result.stdout + "\n")
        except subprocess.CalledProcessError as e:
            self.output_text.insert(tk.END, f"HIS.py çalıştırılamadı: {e}\n")
        except FileNotFoundError as e:
            self.output_text.insert(tk.END, f"Dosya bulunamadı: {e}\n")
